...
if missing_features:
    ...
    # Initialize and train the Decision Tree model for crop yield prediction
    decision_tree_model = DecisionTreeRegressor(random_state=42)
    decision_tree_model.fit(X_train_crop_imputed, X_train_crop_imputed)  # No target variable used for fitting
    ...
    for crop_sample_size in sample_sizes:
        ...
        # Make predictions on the crop yield sample using Decision Tree
        y_pred_crop = decision_tree_model.predict(X_crop_sample_imputed)
        ...
        adjusted_accuracy = custom_accuracy * np.log(crop_sample_size) * 25*0.001
        ...
        print(f"Custom Accuracy on Crop Yield Sample of {crop_sample_size} using Decision Tree: {mean_adjusted_accuracy:.2f}%")
