...
if missing_features:
    ...
    # Initialize and train the Random Forest model for crop yield prediction
    random_forest_model = RandomForestRegressor(n_estimators=100, random_state=42)
    random_forest_model.fit(X_train_crop_imputed, X_train_crop_imputed)  # No target variable used for fitting
    ...
    for crop_sample_size in sample_sizes:
        ...
        # Make predictions on the crop yield sample using Random Forest
        y_pred_crop = random_forest_model.predict(X_crop_sample_imputed)
        ...
        print(f"Custom Accuracy on Crop Yield Sample of {crop_sample_size} using Random Forest: {mean_adjusted_accuracy:.2f}%")
