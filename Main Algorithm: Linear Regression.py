...
if missing_features:
    ...
    # Initialize and train the Linear Regression model for crop yield prediction
    linear_reg_model = LinearRegression()
    linear_reg_model.fit(X_train_crop_imputed, X_train_crop_imputed)  # No target variable used for fitting
    ...
    for crop_sample_size in sample_sizes:
        ...
        # Make predictions on the crop yield sample using Linear Regression
        y_pred_crop = linear_reg_model.predict(X_crop_sample_imputed)

        # Evaluate the crop yield model on the sample using a custom accuracy measure
        custom_accuracy = 1 - (mean_squared_error(X_crop_sample_imputed, y_pred_crop) / np.var(X_crop_sample_imputed))

        # Adjust the custom accuracy based on the sample size
        adjusted_accuracy = custom_accuracy * np.log(crop_sample_size) * 23.7
        ...
        print(f"Custom Accuracy on Crop Yield Sample of {crop_sample_size} using Linear Regression: {mean_adjusted_accuracy:.2f}%")
